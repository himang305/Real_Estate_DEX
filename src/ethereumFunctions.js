...
export function getKyc(address, signer) {
  return new Contract(address,  KYC, signer);
}
...
async function checkKYCs(address, kyccontract) {
  console.log("here_123");
  console.log(kyccontract);

  const kyc_result = await kyccontract.getKycCountryCode(address).then((result) => {
    console.log(result);
      return result;
    }).catch((error) => {
      console.log('Error in KYC check');
      return 0;
    });
    const bigNumber = ethers.BigNumber.from(kyc_result);
...
export async function swapTokens(
  ...
  const iskyc = await checkKYCs(accountAddress, kycContract);
  console.log("kyc", iskyc);

  if(!iskyc) {  throw new Error(" Missing kYC for user");}
